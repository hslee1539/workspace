...
_SINGLE_PIXEL_PNG_BASE64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+cMHsAAAAASUVORK5CYII="

_ICON_DEFINITIONS: Dict[str, Tuple[str, str]] = {
    "favicon.ico": ("image/x-icon", _SINGLE_PIXEL_PNG_BASE64),
    "apple-touch-icon.png": ("image/png", _SINGLE_PIXEL_PNG_BASE64),
    "apple-touch-icon-precomposed.png": ("image/png", _SINGLE_PIXEL_PNG_BASE64),
    "icon.png": ("image/png", _SINGLE_PIXEL_PNG_BASE64),
}

ICON_ASSETS: Dict[str, Tuple[str, bytes]] = {
    name: (media_type, base64.b64decode(data)) for name, (media_type, data) in _ICON_DEFINITIONS.items()
}


def _icon_response(name: str) -> Response:
    try:
        media_type, content = ICON_ASSETS[name]
    except KeyError as exc:
...
async def apple_touch_icon_precomposed() -> Response:
    return _icon_response("apple-touch-icon-precomposed.png")


@app.get("/icon.png", include_in_schema=False)
async def icon_png() -> Response:
    return _icon_response("icon.png")


@app.get("/static/icon.png", include_in_schema=False)
async def legacy_icon_png() -> Response:
    return _icon_response("icon.png")
